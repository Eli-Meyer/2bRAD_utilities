...
print OUT "##fileDate=", 1900+$ta[5], $months[$ta[4]], $days[$ta[3]-1], "\n";
print OUT "##source=",$scriptname, "\n";
if (defined($rfile)) {print OUT "##reference=", $rfile, "\n";}
...
if (defined($ffile))
	...
	while (<FF>)
		...
		chomp;
		@bits = split("\t", $_);
		print OUT "##FILTER=<ID=", $bits[0], ",Description=\"", $bits[1], "\">\n";
		}
	}
...
# print OUT genotype data for each sample
	...
	for ($a=2; $a<$nom; $a++)
		...
		if ($cols[$a] =~ / /)
			{
			@obsall = split(" ", $cols[$a]);
			print OUT $ach{$obsall[0]}, "/", $ach{$obsall[1]}, "\t";
			}
		elsif ($cols[$a] eq 0)
			{
			print OUT "./.", "\t";
			...
			{
			print OUT $ach{$cols[$a]}, "/", $ach{$cols[$a]}, "\t";	
			}
		}
	print OUT "\n";
	}
